...
function reboot(reboot) {
    ...
    if (getValueEnc(yes_no_map, reboot) === 0) {
        return [];
    }
...
function reportStatus(report_status) {
    ...
    if (getValueEnc(yes_no_map, report_status) === 0) {
        return [];
    }
...
function reportAttribute(report_attribute) {
    ...
    if (getValueEnc(yes_no_map, report_attribute) === 0) {
        return [];
    }
...
function socketStatus(socket_status) {
    ...
    buffer.writeUInt8(getValueEnc(on_off_map, socket_status));
...
function socketStatusWithDelay(socket_status, delay_time) {
    ...
    var data = (0x01 << 4) | getValueEnc(on_off_map, socket_status);
...
function setOverCurrentProtection(over_current_protection) {
    ...
    buffer.writeUInt8(getValueEnc(enable_map, enable));
...
function setChildLock(child_lock_config) {
    ...
    var data = (getValueEnc(enable_map, enable) << 15) + lock_time;
    var buffer = new Buffer(4);
    buffer.writeUInt8(0xff);
...
function powerConsumptionEnable(power_consumption_enable) {
    ...
    buffer.writeUInt8(getValueEnc(enable_map, power_consumption_enable));
    return buffer.toBytes();
}
...
function resetPowerConsumption(reset_power_consumption) {
    ...
    if (getValueEnc(yes_no_map, reset_power_consumption) === 0) {
        return [];
    }
...
function setLedEnable(led_indicator_enable) {
    ...
    buffer.writeUInt8(getValueEnc(enable_map, led_indicator_enable));
    return buffer.toBytes();
}
...
function getValueEnc(map, value) {
...
function getHaDeviceInfo() {
    return {
        ...
        entities: {
            ...
            factor: {
                entity_conf: {
                    value_template: "{{ (value_json.object.power_factor | float) / 100 }}",
                    entity_category: "diagnostic",
                    state_class: "measurement",
            ...
            power: {
                entity_conf: {
                    value_template: "{{ value_json.object.active_power | float }}",
                    state_class: "measurement",
                    device_class: "power",
            ...
            outage: {
                ...
                entity_conf: {
                    state: "{{ value_json.object.outage }}",
                    entity_category: "diagnostic"
                }
            },
            power_sum: {
                entity_conf: {
                    value_template: "{{ (value_json.object.power_consumption | float) / 1000 }}",
            ...
            state: {
                integration: "switch",
                entity_conf: {
                    value_template: "{{ value_json.object.socket_status }}",
                    command_topic: "{command_topic}",
                    state_on: "on",
                    state_off: "off",
                    payload_off: '{"dev_eui":"{dev_eui}", "confirmed":true, "fPort":85, "object": { "socket_status" : "off" }}',
                    payload_on: '{"dev_eui":"{dev_eui}", "confirmed":true, "fPort":85, "object": { "socket_status" : "on" }}'
                }
            },
            rssi: {
                entity_conf: {
